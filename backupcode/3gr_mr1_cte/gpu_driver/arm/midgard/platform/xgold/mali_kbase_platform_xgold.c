...
static int kbase_platform_xgold_pm_init(struct kbase_device *kbdev)
{
	...
	plf_context->resume_pm_state = GPU_MAX_PM_STATE;
...
#ifdef CONFIG_MALI_MIDGARD_XGOLD_THERMAL
static int mali_get_max_state(struct thermal_cooling_device *cooling_dev,
				unsigned long *state)
{
	mali_dbg("mali_get_max_state current state=%u\n", GPU_MAX_PM_STATE);

	*state = GPU_MAX_PM_STATE;

	return 0;
}

static int mali_get_cur_state(struct thermal_cooling_device *cooling_dev,
				unsigned long *state)
{
	struct kbase_device *kbdev = cooling_dev->devdata;
	struct xgold_platform_context *plf_context;


	plf_context = (struct xgold_platform_context *) kbdev->platform_context;

	mali_dbg("mali_get_cur_state curr_state=%u\n",
				plf_context->curr_pm_state);

	mutex_lock(&plf_context->pm_lock_mutex);
	*state = plf_context->curr_pm_state;
	mutex_unlock(&plf_context->pm_lock_mutex);

	return 0;
}

static int mali_set_cur_state(struct thermal_cooling_device *cooling_dev,
				unsigned long state)
{
	...
	if (state <= 0 || state > GPU_MAX_PM_STATE) {
		mali_err("Invalid state requested state=%lu\n", state);
		return -EINVAL;
	}
...
#ifdef CONFIG_MALI_MIDGARD_DVFS
	mali_dbg("mali_set_cur_state state=%lu, curr_state=%u, resume_state=%u\n",
	  state, plf_context->curr_pm_state, plf_context->resume_pm_state);
	if (plf_context->curr_pm_state <= state) {
		plf_context->resume_pm_state = state;
		mutex_unlock(&plf_context->pm_lock_mutex);
		return 0;
	}
#endif
...
static const struct thermal_cooling_device_ops mali_cooling_ops = {
...
#ifdef CONFIG_MALI_MIDGARD_XGOLD_THERMAL
	plf_context->cooling_dev = thermal_cooling_device_register("GPU",
					kbdev, &mali_cooling_ops);
	if (IS_ERR(plf_context->cooling_dev)) {
		mali_err("cooling device registration failed\n");
		return MALI_ERROR_FUNCTION_FAILED;
	}
#endif
...
#ifdef CONFIG_MALI_MIDGARD_XGOLD_THERMAL
	thermal_cooling_device_unregister(plf_context->cooling_dev);
#endif
	mutex_lock(&plf_context->pm_lock_mutex);
