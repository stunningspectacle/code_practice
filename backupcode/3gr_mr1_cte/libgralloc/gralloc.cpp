...
int gralloc_alloc_hwbuffer(alloc_device_t* dev, size_t size, int usage, buffer_handle_t* pHandle, int flags)
{
	...
	int                ret = 0;
	int         cache_flag = 0;
	int ion_heap_mask = ION_HEAP_TYPE_DMA_MASK;
	...
	if (usage & GRALLOC_USAGE_PRIVATE_2) {
		ion_heap_mask = ION_HEAP_TYPE_SECURE_MASK;
		ALOGI("[%s] VPU buffers routed to secured heap\n", __func__);
	}
        ...
        buffer_handle_t* pHandle, int* pStride)
{
	...
	if (HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED == format) {
		...
		ALOGV("[%s] HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED\n", __func__);
	}
