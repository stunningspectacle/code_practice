...
{
    size_t stride, bpr, y_size, c_size, c_stride;
    int bpp = 0;
    int err;
    char* s;
...
static void hal_to_dcc_bounds(const hwc_rect_t* halr, struct dcc_bounds_t* dccr) {
    dccr->x = halr->left;
    dccr->y = halr->top;
...
static void hal_to_dcc_layer_back(const hwc_layer_1_t* hal_layer,
    ...
    hal_to_dcc_bounds(&hal_layer->sourceCrop, &dcc_layer->src);
...
int dcc_hal_compose(hwc_layer_1_t* fb_layer, hwc_layer_1_t** ov_layers,
    ...
    if (!fb_enabled) {
        ...
        updt.back.fd   = -1;
        updt.flags |= DCC_UPDATE_NOBG_MASK;
        updt.back.phys = 0;
        updt.back.fmt = DCC_FMT_ARGB8888;
    }
    ...
    DCC_IOCTL(err, dcc_fd, DCC_IOW_COMPOSE, &updt);
