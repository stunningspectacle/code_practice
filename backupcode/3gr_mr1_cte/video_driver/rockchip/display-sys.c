...
int display_create_sysfs(struct device *dev)
{
	int r, t;

	for (t = 0; t < ARRAY_SIZE(display_attrs); t++) {
		r = device_create_file(dev, &display_attrs[t]);
		if (r) {
...
void display_remove_sysfs(struct device *dev)
{
	int t;

	for (t = 0; t < ARRAY_SIZE(display_attrs); t++)
		device_remove_file(dev, &display_attrs[t]);
}

void rk_display_device_enable(struct rk_display_device *ddev)
{
	struct list_head *pos, *head = &display_device_list;
	struct rk_display_device *dev = NULL;
	struct rk_display_device *dev_enabled = NULL, *dev_enable = NULL;
			   ...
			   struct device *parent, void *devdata)
{
	...
	if (likely(new_dev) && unlikely(driver->probe(new_dev, devdata))) {
		...
		new_dev->idx = idr_alloc(&allocated_dsp,
					 new_dev, 0, 0, GFP_KERNEL);
		...
		if (new_dev->idx >= 0) {
			...
			display_create_sysfs(new_dev->dev);
