...
static int hdmi_edid_parse_cea_sad(unsigned char *buf, struct hdmi_edid *pedid)
{
	...
	count = buf[0] & 0x1F;
	pedid->audio = kmalloc((count / 3) * sizeof(*pedid->audio),
				GFP_KERNEL);
	if (pedid->audio == NULL)
		return E_HDMI_EDID_NOMEMORY;
...
static int hdmi_edid_parse_cea_sdb(unsigned char *buf, struct hdmi_edid *pedid)
{
	...
	hdmi_edid_debug("[EDID-CEA] CEC Physical address is 0x%08x.\n",
			pedid->cecaddress);
	...
	if (count > 5) {
		...
		hdmi_edid_debug("dc_48bit%d dc_36bit%d dc_30bit%d dc_y444 %d\n",
					  ...
					  struct hdmi_edid *pedid)
{
	...
	while (cur_offset < ddc_offset) {
		...
		hdmi_edid_debug("[EDID-CEA] It is a ");
		switch (tag) {
		case 0x02:
			hdmi_edid_debug("Video Data Block.\n");
			/* Video Data Block */
			hdmi_edid_get_cea_svd(buf + cur_offset, pedid);
			break;
		case 0x01:
			/* Audio Data Block */
			hdmi_edid_debug("Audio Data Block.\n");
			hdmi_edid_parse_cea_sad(buf + cur_offset, pedid);
			break;
		case 0x04:
			/* Speaker Allocation Data Block */
			hdmi_edid_debug("Speaker Allocation Data Block.\n");
			break;
		case 0x03:
			/* Vendor Specific Data Block */
			hdmi_edid_debug("Vendor Specific Data Block.\n");
			hdmi_edid_parse_cea_sdb(buf + cur_offset, pedid);
			break;
		case 0x05:
			/* VESA DTC Data Block */
			hdmi_edid_debug("VESA DTC Data Block.\n");
			break;
		case 0x07:
			/* Use Extended Tag */
			hdmi_edid_debug("Use Extended Tag Data Block.\n");
			break;
		default:
			hdmi_edid_error("unkowned data block tag.\n");
			break;
		}
				      ...
				      struct hdmi_edid *pedid)
{
	...
	hdmi_edid_debug("[EDID-EXTEND] It is a ");
	switch (buf[0]) {
	case 0xF0:
		hdmi_edid_debug("extensions block map.\n");
		break;
	case 0x02:
		hdmi_edid_debug("CEA 861 Series Extension.\n");
		hdmi_edid_parse_extensions_cea(buf, pedid);
		break;
	case 0x10:
		hdmi_edid_debug("Video Timing Block Extension.\n");
		break;
	case 0x40:
		hdmi_edid_debug("Display Information Extension.\n");
		break;
	case 0x50:
		hdmi_edid_debug("Localized String Extension.\n");
		break;
	case 0x60:
		hdmi_edid_debug("Digital Packet Video Link Extension.\n");
		break;
	default:
		hdmi_edid_error("Unkowned extension.\n");
...
int hdmi_sys_parse_edid(struct hdmi *hdmi)
{
	...
	int rc = HDMI_ERROR_SUCCESS, extendblock = 0, i;
