...
static void hdmi_sys_show_state(struct hdmi *hdmi)
{
	...
	case PLAY_BACK:
		...
		break;
	default:
		hdmi_dbg(hdmi->dev, "Unknown State %d\n", hdmi->state);
		break;
	}
}
...
{
	if (uboot_vic > 0) {
		hdmi->uboot_logo = support_loader_display();
...
void hdmi_sys_remove(struct hdmi *hdmi)
{
	...
	hdmi->display = HDMI_DISABLE;
	if (hdmi->set_vif)
		hdmi->set_vif(hdmi, hdmi->vop->cur_screen, 0);
	rockchip_fb_switch_screen(hdmi->vop->cur_screen, false, hdmi->vop->id);
#ifdef CONFIG_SWITCH
	/*if (audio_need)*/
...
void hdmi_work(struct work_struct *work)
{
	...
	int rc = HDMI_ERROR_SUCCESS, trytimes = 0;
	...
	do {
		...
		case READ_PARSE_EDID:
			...
			if (rc == HDMI_ERROR_SUCCESS) {
				if (hdmi->cec_set_device_pa) {
					uint32_t addr = hdmi->edid.cecaddress;

					hdmi->cec_set_device_pa(addr);
				}
