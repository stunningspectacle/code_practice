...
#define IS_YUV_420(format) \
	...
	(format == RGA_FORMAT_YCRCB_420_P) | \
	(format == RGA_FORMAT_YCRCB_420_SP))
#define IS_YUV_422(format) \
	((format == RGA_FORMAT_YCBCR_422_P) | \
	(format == RGA_FORMAT_YCBCR_422_SP) | \
	(format == RGA_FORMAT_YCRCB_422_P) | \
	(format == RGA_FORMAT_YCRCB_422_SP))
...
int32_t rga_gen_two_pro(struct rga_req *msg, struct rga_req *msg1)
{
	...
	mp = msg1;
	if (msg->dst.act_w == 0) {
		pr_info("%s, [%d] rga dst act_w is zero\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	if (msg->dst.act_h == 0) {
