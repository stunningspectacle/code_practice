...
{
	int err = sync_fence_wait(fence, 10000);

	if (err >= 0)
		return;

	if ((err == -ETIME) || (err = -ERESTARTSYS)) {
		pr_err("relaunch due to err=%d\n", err);
			  ...
			  bool enable, int vop_id)
{
	...
	if(enable)
		intel_rockchip_disable_panel(dev_drv);
