...
void intel_rockchip_disable_panel(struct rockchip_vop_driver *dev_drv)
{
	struct vop_device *vop_dev = container_of(dev_drv, struct vop_device, driver);
	struct rockchip_screen *screen = dev_drv->cur_screen;
	...
	switch (screen->type)
	{
		...
		case SCREEN_LVDS:
		case SCREEN_HDMI:
			if (screen->index >= 0 && dev_drv->trsm_ops && dev_drv->trsm_ops->disable)
				dev_drv->trsm_ops->disable();
			break;
		default:
			dev_err(vop_dev->dev, "un supported panel interface !\n");
			break;
	}
}
