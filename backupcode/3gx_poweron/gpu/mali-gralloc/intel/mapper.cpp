...
static int gralloc_map(gralloc_module_t const* module, buffer_handle_t handle)
{
    ...
    void* mappedAddress = mmap(0, hnd->size, PROT_READ | PROT_WRITE, MAP_SHARED, hnd->fd, 0);
    if (mappedAddress == MAP_FAILED) {
        ALOGE("%s: could not mmap %s", __func__, strerror(errno));
        return -errno;
    }
    ALOGV("%s: base %p %d %d %d\n", __func__, mappedAddress, hnd->size, hnd->w, hnd->h);
        ...
        struct android_ycbcr *ycbcr)
{
    ...
    switch (hnd->fmt) {
        ...
        case HAL_PIXEL_FORMAT_YV12:
            ...
            ycbcr->y  = (void*)hnd->base;
            ycbcr->cr = (void*)(hnd->base + ystride * hnd->h);
            ycbcr->cb = (void*)(hnd->base + ystride * hnd->h + ycbcr->cstride * hnd->h/2);
            ycbcr->chroma_step = 1;
            memset(ycbcr->reserved, 0, sizeof(ycbcr->reserved));
            break;
        ...
        case HAL_PIXEL_FORMAT_YCbCr_420_SP:
            ystride = hnd->byte_stride;
            ycbcr->y  = (void*)hnd->base;
            ycbcr->cb = (void*)(hnd->base + ystride * hnd->h);
            ycbcr->cr = (void*)(hnd->base + ystride * hnd->h + 1);
            ycbcr->ystride = ystride;
            ycbcr->cstride = ystride;
            ycbcr->chroma_step = 2;
            memset(ycbcr->reserved, 0, sizeof(ycbcr->reserved));
            break;
        //TODO:add a full list support for all formats.
        default:
            ystride = hnd->byte_stride;
            ycbcr->y  = (void*)hnd->base;
            ycbcr->cr = (void*)(hnd->base + ystride * hnd->h);
            ...
            ycbcr->chroma_step = 2;
            memset(ycbcr->reserved, 0, sizeof(ycbcr->reserved));
            ALOGW("%s: Invalid format passed: 0x%x", __FUNCTION__, hnd->fmt);
            break;
    }
    return 0;
}
