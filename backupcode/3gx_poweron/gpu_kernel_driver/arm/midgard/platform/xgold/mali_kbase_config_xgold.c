...
static void pm_callback_power_off(struct kbase_device *kbdev)
{
	...
	if (kbdev->pm.pm_current_policy == NULL ||
			!(strcmp((kbdev->pm.pm_current_policy->name),
						"always_on"))) {
		struct xgold_platform_context *plf_context;

		mali_dbg("%s()inside always_on case\n", __func__);

		plf_context =
		(struct xgold_platform_context *) kbdev->platform_context;
		mutex_lock(&plf_context->pm_lock_mutex);

#ifdef CONFIG_MALI_MIDGARD_DVFS
		if (!plf_context->dvfs_off && kbdev->pm.metrics.kbdev != NULL) {
			mali_dbg("%s()inside DVFS disable\n", __func__);
			kbase_platform_dvfs_enable(MALI_FALSE, kbdev);
		}
#endif
		kbase_platform_xgold_pm_control(kbdev, MALI_PLF_PM_STATE_D3);
...
#endif
		kbase_platform_xgold_pm_control(kbdev,
					plf_context->resume_pm_state);
		mutex_unlock(&plf_context->pm_lock_mutex);
		return 1;
	}
