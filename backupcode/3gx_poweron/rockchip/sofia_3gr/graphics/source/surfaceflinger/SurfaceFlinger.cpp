...
void SurfaceFlinger::updateCursorAsync()
{
    ...
    for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
        ...
        for (size_t i=0 ; cur!=end && i<count ; ++i, ++cur) {
	    HWComposer::HWCLayerInterface *curLayerInterface = &*cur;
            if (curLayerInterface && curLayerInterface->getCompositionType() != HWC_CURSOR_OVERLAY) {
                continue;
            }
            const sp<Layer>& layer(currentLayers[i]);
...
bool SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    if (cur != end) {
        ...
        for (size_t i=0 ; i<count && cur!=end ; ++i, ++cur) {
            ...
            HWComposer::HWCLayerInterface *curLayerInterface = &*cur;
            if (!clip.isEmpty() && curLayerInterface ) {
                switch (curLayerInterface->getCompositionType()) {
                    case HWC_CURSOR_OVERLAY:
                    case HWC_OVERLAY: {
                        const Layer::State& state(layer->getDrawingState());
                        if ((curLayerInterface->getHints() & HWC_HINT_CLEAR_FB)
                                && i
