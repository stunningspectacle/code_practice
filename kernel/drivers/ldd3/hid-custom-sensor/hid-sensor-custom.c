...
#define HID_CUSTOM_MAX_INPUT		29
...
struct hid_sensor_custom {
	...
	int input_field_index[HID_CUSTOM_MAX_INPUT];
				 ...
				 unsigned usage_id, void *priv)
{
	...
	if (atomic_read(sensor_inst->common_attrib.data_ready) &&
			(kfifo_avail(&sensor_inst->data_fio) >=
		...
		dev_dbg(&indio_dev->dev, "push data to iio\n", sample->raw_len);
				       ...
				       struct hid_field *field)
{
	...
	if (report_type == HID_FEATURE_REPORT)
		...
		snprintf(sensor_field->group_name,
			 ...
			 sensor_field->attribute.attrib_id);
		if (sensor_inst->input_field_count >= HID_CUSTOM_MAX_INPUT) {
			dev_err(&sensor_inst->common_attrib.pdev->dev,
		...
		/* Add one iio chann for this input report */
		iio_channs = krealloc(sensor_inst->iio_channs,
				(sensor_inst->input_field_count + 1) *
								...
								*sensor_inst)
{
	...
	for (i = 0; i < sensor_inst->sensor_field_count; ++i) {
		...
		ret = sysfs_create_group(
				&sensor_inst->common_attrib.pdev->dev.kobj,
								...
								sensor_inst)
{
	...
	for (i = 0; i < sensor_inst->sensor_field_count; ++i)
		sysfs_remove_group(&sensor_inst->common_attrib.pdev->dev.kobj,
				   &sensor_inst->fields[i].
				   hid_custom_attribute_group);
		...
		int *val, int *val2, long mask)
{
	...
	u32 address;
	int report_id = -1;
	struct hid_sensor_custom *sensor_inst = iio_priv(indio_dev);
	struct hid_sensor_hub_device *hsdev = sensor_inst->common_attrib.hsdev;
	int field_index = sensor_inst->input_field_index[chan->scan_index];
	...
	*val2 = 0;
	...
	case IIO_CHAN_INFO_RAW:
		...
		report_id = attrib->report_id;
		min = attrib->logical_minimum;
		if (report_id >= 0)
			*val = sensor_hub_input_attr_get_raw_value(
					hsdev, hsdev->usage,
					chan->scan_index, report_id,
					SENSOR_HUB_SYNC,
					min < 0);
		else {
			hid_sensor_power_state(&sensor_inst->common_attrib,
						 false);
			return -EINVAL;
		}
	...
	case IIO_CHAN_INFO_SCALE:
		/* FIXME: change to sensor's config */
		*val = 1;
		*val2 = 0;
		ret_type = IIO_VAL_INT;
		break;
...
static int hid_sensor_custom_probe(struct platform_device *pdev)
{
	...
	ret = sysfs_create_group(&sensor_inst->common_attrib.pdev->dev.kobj,
	...
	indio_dev->num_channels = sensor_inst->input_field_count;
	indio_dev->channels = devm_kmemdup(&pdev->dev, sensor_inst->iio_channs,
		sensor_inst->input_field_count * sizeof(struct iio_chan_spec),
			GFP_KERNEL);
	...
	indio_dev->name = "hid_custom_sensor";
