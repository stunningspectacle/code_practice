...
struct stack_char { char elements[50]; int top; void (*push)(struct stack_char *, char); char (*pop)(struct stack_char *); char (*peek)(struct stack_char *); int (*isEmpty)(struct stack_char *); void (*dump)(); }; struct stack_char stack; void push_char(struct stack_char *stack, char element) { stack->elements[stack->top++] = element;}char pop_char(struct stack_char *stack) { return stack->elements[--stack->top];}char peek_char(struct stack_char *stack) { return stack->elements[stack->top - 1];}int isEmpty_char(struct stack_char *stack) { if (stack->top == 0) return 1; return 0;}void dump_stack_char(struct stack_char *stack) { int i; for (i = 0; i < stack->top; i++) { if (sizeof(char) == 1) { printf("%c ", stack->elements[i]); } else { printf("%d ", stack->elements[i]); } } printf("\n");}
...
void main(void) {
 ...
 struct heap *h = malloc(sizeof(struct heap) + size * sizeof(struct heapEntry));
 printf("heap size = %d\n", sizeof(struct heap));
 ...
 for (i = 0; i < size; i++) {
  printf("elements[%d]: n1=%d, n2=%d\n", i, h->elements[i].n1, h->elements[i].n2);
 }
}
