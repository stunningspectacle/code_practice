...
#if (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_Intel_ISH)
	#define TRACE_ALLOC_CRITICAL_SECTION() int __irq_status;
	#define TRACE_ENTER_CRITICAL_SECTION() {__irq_status = portSET_INTERRUPT_MASK_FROM_ISR();}
	#define TRACE_EXIT_CRITICAL_SECTION() {portCLEAR_INTERRUPT_MASK_FROM_ISR(__irq_status);}
#endif
 ...
 ******************************************************************************/
#define EVENTGROUP_CREATE_OBJ_SUCCESSFUL	(EVENTGROUP_OBJCLOSE_PROP + 8UL)	/*0x18*/
 ...
 * or mutex).
 ******************************************************************************/
#define EVENTGROUP_SEND_SUCCESSFUL	(EVENTGROUP_CREATE_OBJ_SUCCESSFUL + 8UL)		/*0x20*/
 ...
 * or mutex).
 ******************************************************************************/
#define EVENTGROUP_RECEIVE_SUCCESSFUL	(EVENTGROUP_SEND_SUCCESSFUL + 8UL)		/*0x28*/

/* Send/Give operations, from ISR */
#define EVENTGROUP_SEND_FROM_ISR_SUCCESSFUL \
									(EVENTGROUP_RECEIVE_SUCCESSFUL + 8UL)	/*0x30*/

/* Receive/Take operations, from ISR */
#define EVENTGROUP_RECEIVE_FROM_ISR_SUCCESSFUL \
							(EVENTGROUP_SEND_FROM_ISR_SUCCESSFUL + 8UL)		/*0x38*/

/* "Failed" event type versions of above (timeout, failed allocation, etc) */
#define EVENTGROUP_KSE_FAILED \
							(EVENTGROUP_RECEIVE_FROM_ISR_SUCCESSFUL + 8UL)	/*0x40*/
...
#define EVENTGROUP_PEEK_SUCCESSFUL	(EVENTGROUP_SEND_BLOCK + 8UL)				/*0x78*/

/* Events on object delete (vTaskDelete or vQueueDelete) */
#define EVENTGROUP_DELETE_OBJ_SUCCESSFUL	(EVENTGROUP_PEEK_SUCCESSFUL + 8UL)	/*0x80*/

/* Other events - object class is implied: TASK */
#define EVENTGROUP_OTHERS	(EVENTGROUP_DELETE_OBJ_SUCCESSFUL + 8UL)			/*0x88*/
...
#define traceQUEUE_SEND( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(SEND, SUCCESSFUL, TRC_UNUSED, pxQueue); \
...
#define traceQUEUE_RECEIVE( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, SUCCESSFUL, TRC_UNUSED, pxQueue); \
...
#define traceQUEUE_PEEK( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, SUCCESSFUL, TRC_UNUSED, pxQueue);

/* Called when a message is sent from interrupt context, e.g., using xQueueSendFromISR */
#undef traceQUEUE_SEND_FROM_ISR
#define traceQUEUE_SEND_FROM_ISR( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(SEND_FROM_ISR, SUCCESSFUL, TRC_UNUSED, pxQueue); \
...
#define traceQUEUE_RECEIVE_FROM_ISR( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE_FROM_ISR, SUCCESSFUL, TRC_UNUSED, pxQueue); \
	trcKERNEL_HOOKS_SET_OBJECT_STATE(TRC_UNUSED, pxQueue, (uint8_t)(pxQueue->uxMessagesWaiting - 1));
