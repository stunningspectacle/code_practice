...
/*****************************************************************************
* vTraceCopyTraceBuffer(void *dest)
* Used in smhi task to get the trace data in stream mode. 
* void *dest is used to save the trace data. 
* int32_t destsize is the size of dest. For smhi, it's from smhi.h  SMHI_MAX_PAYLOAD_SIZE.
* return the real bytes copied.
******************************************************************************/
int32_t vTraceCopyTraceBuffer(void* dest, int32_t destsize);
...
#define trcKERNEL_HOOKS_TASK_DELETE(SERVICE, pxTCB) \
	prvTraceStoreKernelCall(TRACE_GET_TASK_EVENT_CODE(SERVICE, SUCCESSFUL, CLASS, pxTCB), TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB)); \
...
#define trcKERNEL_HOOKS_OBJECT_DELETE(SERVICE, CLASS, pxObject) \
	prvTraceStoreKernelCall(TRACE_GET_OBJECT_EVENT_CODE(SERVICE, SUCCESSFUL, CLASS, pxObject), TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject), TRACE_GET_OBJECT_NUMBER(CLASS, pxObject)); \
...
/* copy the trace data to a dest buffer. */
