...
#define configSUPPORT_STATIC_ALLOCATION			0 // since no tracealyzer stream task is created for heci, no need for this. 	
#endif

 __pinned_kernel_bss__ static void* pCurrentTCB = NULL;
 __pinned_kernel_bss__ static xTaskHandle HandleTzCtrl = NULL;       /* TzCtrl task TCB */
...
#ifndef TRC_RECORDER_TRANSFER_METHOD_HECI
/* The TzCtrl task - receives commands from Tracealyzer (start/stop) */
static portTASK_FUNCTION( TzCtrl, pvParameters );
#endif /* ndef TRC_RECORDER_TRANSFER_METHOD_HECI */
...
#ifndef TRC_RECORDER_TRANSFER_METHOD_HECI // if heci, no need to create task.
...
#ifdef TRC_RECORDER_TRANSFER_METHOD_HECI
/**************************************************************************
To enable the trace log sending in other task, like SMHI. No task is created here.
The trace data is ready from SMHI task and send out.
***************************************************************************/
int32_t vTraceCopyTraceBuffer(void* dest, int32_t destsize)
{
	int32_t bytes = 0;
	TRC_STREAM_PORT_PERIODIC_SEND_DATA(&bytes,dest,destsize);
	prvCheckRecorderStatus();
	return bytes;
}
#endif
/*****************************************************************************
End of sending out trace from SMHI
*****************************************************************************/
 ...
 ******************************************************************************/
__pinned_kernel_code__ uint32_t prvIsNewTCB(void* pNewTCB)
{
