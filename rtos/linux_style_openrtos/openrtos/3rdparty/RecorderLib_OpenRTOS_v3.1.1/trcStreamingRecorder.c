...
__pinned_kernel_data__  static SymbolTable symbolTable = { { { 0 } } };

/* This points to the first unused entry in the symbol table. */
__pinned_kernel_data__  static uint32_t firstFreeSymbolTableIndex = 0;

/* The Object Data Table instance - keeps initial priorities of tasks. */
__pinned_kernel_data__ static ObjectDataTable objectDataTable = { { { 0 } } };

/* This points to the first unused entry in the object data table. */
__pinned_kernel_data__ static uint32_t firstFreeObjectDataTableIndex = 0;

/* Keeps track of ISR nesting */
__pinned_kernel_data__ static uint32_t ISR_stack[TRC_CFG_MAX_ISR_NESTING];

/* Keeps track of ISR nesting */
__pinned_kernel_data__ static int8_t ISR_stack_index = -1;
...
__pinned_kernel_data__ uint32_t RecorderEnabled = 0u;
...
__pinned_kernel_data__ static uint32_t eventCounter = 0;
...
__pinned_kernel_data__ int32_t isPendingContextSwitch = 0;

__pinned_kernel_data__ uint32_t uiTraceTickCount = 0;
__pinned_kernel_data__ uint32_t timestampFrequency = 0;
__pinned_kernel_data__ uint32_t DroppedEventCounter = 0;	// Total number of dropped events (failed allocations)
__pinned_kernel_data__ uint32_t TotalBytesRemaining_LowWaterMark = TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT * TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE;
__pinned_kernel_data__ uint32_t TotalBytesRemaining = TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT * TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE;

__pinned_kernel_bss__ PageType PageInfo[TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT];
 ...
 ******************************************************************************/
__pinned_kernel_data__ volatile uint32_t NoRoomForObjectData = 0;
 ...
 ******************************************************************************/
__pinned_kernel_code__  traceString xTraceRegisterString(const char* name)
{
 ...
 *
 ******************************************************************************/
__pinned_kernel_code__  void vTracePrint(traceString chn, const char* str)
{
 ...
 *
 ******************************************************************************/
__pinned_kernel_code__  void vTracePrintF(traceString chn, const char* fmt, ...)
{
	va_list vl;
	int i = 0;
 ...
 *
 ******************************************************************************/
 __pinned_kernel_code__ traceHandle xTraceSetISRProperties(const char* name, uint8_t priority)
{
 ...
 *
 ******************************************************************************/
__pinned_kernel_code__ void vTraceStoreISRBegin(traceHandle handle)
{
 ...
 *
 ******************************************************************************/
__pinned_kernel_code__ void vTraceStoreISREnd(int isTaskSwitchRequired)
{
	TRACE_ALLOC_CRITICAL_SECTION();
...
#ifdef TRC_RECORDER_TRANSFER_METHOD_HECI
	if(isEnabled!=RecorderEnabled) {
#endif
...
__pinned_kernel_code__ void prvTraceStoreEvent1(uint16_t eventID, uint32_t param1)
{
  	TRACE_ALLOC_CRITICAL_SECTION();
...
__pinned_kernel_code__ void prvTraceStoreEvent2(uint16_t eventID, uint32_t param1, uint32_t param2)
{
  	TRACE_ALLOC_CRITICAL_SECTION();
...
__pinned_kernel_code__ void prvTraceStoreStringEvent(int nArgs, uint16_t eventID, const char* str, ...)
{
  	va_list vl;
...
/* Internal common function for storing string events */
__pinned_kernel_code__  static void prvTraceStoreStringEventHelper(	int nArgs,
										uint16_t eventID,
										traceString userEvtChannel,
...
__pinned_kernel_code__ void prvTraceSaveSymbol(const void *address, const char *name)
{
	uint32_t i;
	uint32_t foundSlot;
	uint32_t *ptrAddress;
...
/* Saves an object data entry (current task priority) in object data table */
 __pinned_kernel_code__ void prvTraceSaveObjectData(const void *address, uint32_t data)
{
	uint32_t i;
	uint32_t foundSlot;
	uint32_t *ptr;
...
/* Retrieve a buffer page to write to. */
__pinned_kernel_code__ static int prvAllocateBufferPage(int prevPage)
{
	int index;
	int count = 0;
...
*******************************************************************************/
#ifdef TRC_RECORDER_TRANSFER_METHOD_HECI
int32_t prvPagedEventBufferTransfer(int32_t (*writeFunc)(void* data, uint32_t size, int32_t* ptrBytesWritten, void* destdata, int32_t destsize), int32_t* nofBytes, void* destdata, int32_t destsize)
#else
...
#endif
{
...
#ifdef TRC_RECORDER_TRANSFER_METHOD_HECI
        if(size>0) break;
		else vTaskDelay(TRC_CFG_CTRL_TASK_DELAY);	/* 10ms */
	}    
#endif
    ...
    if (pageToTransfer > -1)
    {
        ...
        {
#ifdef TRC_RECORDER_TRANSFER_METHOD_HECI
            if (writeFunc(&EventBuffer[pageToTransfer * TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE + transferred], size - transferred, nofBytes, destdata, destsize) == 0)
#else
...
*******************************************************************************/
__pinned_kernel_code__ void* prvPagedEventBufferGetWritePointer(int sizeOfEvent)
{
	void* ret;
	__pinned_kernel_data__ static int currentWritePage = -1;
