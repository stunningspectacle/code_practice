...
#if (ISH_CONFIG_NUM_I3C_DEVICES > 0)
	LEVEL_INTR(I3C0_IRQ,            I3C0_VEC),
#endif
...
__init_flow_code__ void registerVecToHandler(void)
{
...
#if (ISH_CONFIG_NUM_I3C_DEVICES > 0)
	xPortRegisterCInterruptHandler(i3c_isr_bus0, I3C0_VEC);
#endif
    ...
    xPortRegisterCInterruptHandler(sbep_isr, SBEP_VEC);
...
// Workaround for D0i3 issue which caused by UART Rx pins on LKF A1
// Rx pins needs to be high to make UART get into idle
#define MISC_REV_ID                 (MISC_BASE + 0x100)
#define REV_ID_A1                   0xA
#define MISC_ECO_REG_RW             (MISC_BASE + 0x20C)
#define UART0_RX_HIGH               (1 << 0)
#define UART1_RX_HIGH               (1 << 1)
#define UART2_RX_HIGH               (1 << 2)

__pinned_kernel_code__ static void uart_to_idle()
{
    if (read32(MISC_REV_ID) == REV_ID_A1) {
        write32(MISC_ECO_REG_RW,
                UART0_RX_HIGH | UART1_RX_HIGH | UART2_RX_HIGH);
    }
...
#if (ISH_CONFIG_NUM_I3C_DEVICES > 0)
	mmuMap(I3C0_BASE,   I3C0_BASE_PHYS,     I3C0_SIZE,  MMIO_PTE_FLAGS);
#endif
