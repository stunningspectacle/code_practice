...
/**
 * INTEL CHANGE: Defines the priority used by default.  This must not be modified.
 */
#define tskDEFAULT_PRIORITY			tskIDLE_PRIORITY + 1
...
/*
 * INTEL CHANGE: get current TCB top of stack
 */
StackType_t* getTopOfStack();
/*
 * Get the maximum size which ever been used in stack, this can be used to get optimal stack size when create task.
 * Suggest call this function after finished required test cases.
 * @param xTask Handle to the target task.
 * @param stackSize  the stack size when create task specified by xTask.
 * @return  maximum size ever been used in stack.
 */
BaseType_t xTaskGetStackUsedSize( const TaskHandle_t xTask, unsigned int stackSize );
