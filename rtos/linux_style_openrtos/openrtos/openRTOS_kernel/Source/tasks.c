...
PRIVILEGED_DATA TCB_t * volatile pxFpTcbOwner = NULL;
...
BaseType_t xTaskGetStackUsedSize( const TaskHandle_t xTask, unsigned int stackSize )
{
	...
	uxReturn = uxTaskGetStackHighWaterMark(xTask) * 4;

	return (stackSize - uxReturn);
}
#else
#define xTaskGetStackUsedSize( const TaskHandle_t xTask, unsigned int stackSize )	(0)
#endif
...
1. */
	...
	__pinned_kernel_code__ void vTaskStepTick( const TickType_t xTicksToJump )
	{
...
__pinned_kernel_code__ BaseType_t xTaskIncrementTick( void )
{
TCB_t * pxTCB;
...
__pinned_kernel_code__ void vTaskSwitchContext( void )
{
	if( uxSchedulerSuspended != ( UBaseType_t ) pdFALSE )
	{
	...
	else
	{
		...
		/* Check if new task uses FPU and is not equals to previous FP owner
		 * in order to fnsave the previous owner and restore the new task's FP
		 * registers.
		 */
		if(pxCurrentTCB->fpContextPtr)
		{
			//Enable FP (clear CR0[TS]).
			__asm__ volatile ("clts\n\t");
			if(pxFpTcbOwner != pxCurrentTCB)
			{
				if(pxFpTcbOwner)
				{
					//Save the FPU context of the previous thread
					if (pxFpTcbOwner->fpContextPtr)
					{
					...
					__asm__ volatile (
							"frstor (%0);\n\t"
							:
							: "r" (pxCurrentTCB->fpContextPtr) : "memory"
							);
		...
		else //if the current task doesn't use FPU - set the TS
		{
		    void *tempReg;
...
TCB_t *pxNewTCB;
	...
	#if( portSTACK_GROWTH > 0 )
	{
		...
		if( pxNewTCB != NULL )
		{
			...
			pxNewTCB->pxStack = ( StackType_t * ) pvPortStackAllocAligned( ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ), puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
	...
	StackType_t *pxStack;
		...
		/* Allocate space for the stack used by the task being created. */
		pxStack = ( StackType_t * ) pvPortStackAllocAligned( ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ), puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
...
#ifdef ISH_CONFIG_SUPPORT_DEMAND_PAGING
				// INTEL CHANGE: we must pin stacks to avoid stack evictions by page manager.
				extern int pin_buf(const void *addr, size_t len);
				pin_buf(pxNewTCB, sizeof( pxNewTCB ));
				pin_buf(pxNewTCB->pxStack, usStackDepth * sizeof( StackType_t ));
#endif
			}
			else
			{
...
/*-----------------------------------------------------------*/
	...
	static void prvDeleteTCB( TCB_t *pxTCB )
	{
		...
		#if( portUSING_MPU_WRAPPERS == 1 )
		{
			...
			{
				vPortStackFreeAligned( pxTCB->pxStack );
			}
		}
		#else
		{
...
__pinned_kernel_code__ static void prvResetNextTaskUnblockTime( void )
{
TCB_t *pxTCB;
...
StackType_t* getTopOfStack(void)
{
extern uint32_t ulSystemStack[ configISR_STACK_SIZE ];

    return (pxCurrentTCB != NULL) ? (StackType_t*)pxCurrentTCB->pxStack : (StackType_t*)ulSystemStack;
}
