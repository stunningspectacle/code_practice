...
cscope-linux-kernel() {
    ...
    find $LNX \
    -path "$LNX/kernel-next" -prune -o \
    -path "$KERNEL/.git" -prune -o \
    -path "$KERNEL/arch/*" ! -path "$KERNEL/arch/x86*" -prune -o \
    -path "$KERNEL/Documentation" -prune -o \
    -path "$KERNEL/tools" -prune -o \
    -path "$KERNEL/scripts" -prune -o \
    -path "$KERNEL/leo" -prune -o \
    -path "$MODULES/drivers/.git" -prune -o \
    -type l -prune -o \
    -name ".*" -prune -o \
